...
-- ============================================================
-- 6. Add new notification types to enum
-- ============================================================

ALTER TYPE notification_type ADD VALUE IF NOT EXISTS 'service_run_created';
ALTER TYPE notification_type ADD VALUE IF NOT EXISTS 'service_run_completed';
ALTER TYPE notification_type ADD VALUE IF NOT EXISTS 'service_run_cancelled';
...
-- ============================================================
-- 10. Realtime support for new tables
-- ============================================================

-- Add new tables to realtime publication
DO $$
BEGIN
  ALTER PUBLICATION supabase_realtime ADD TABLE d2.services;
EXCEPTION
...
DO $$
BEGIN
  ALTER PUBLICATION supabase_realtime ADD TABLE d2.service_runs;
EXCEPTION
  WHEN duplicate_object THEN NULL;
END $$;

-- Set REPLICA IDENTITY FULL so Realtime broadcasts all columns on changes
-- Without this, realtime only sends primary key, breaking filtered subscriptions
ALTER TABLE d2.services REPLICA IDENTITY FULL;
ALTER TABLE d2.service_runs REPLICA IDENTITY FULL;

-- Grant SELECT to authenticated users so Realtime can deliver events
-- (RLS policies still apply for row-level filtering)
GRANT SELECT ON d2.services TO authenticated;
GRANT SELECT ON d2.service_runs TO authenticated;
